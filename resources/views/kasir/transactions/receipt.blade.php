    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
        <a href="{{ route('kasir.transactions.create') }}"
           class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10.75 4.75a.75.75 0 0 0-1.5 0v4.5h-4.5a.75.75 0 0 0 0 1.5h4.5v4.5a.75.75 0 0 0 1.5 0v-4.5h4.5a.75.75 0 0 0 0-1.5h-4.5v-4.5Z" clip-rule="evenodd"/></svg>
            Transaksi Baru
        </a>
    ...
    <div id="print-area" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-1.5 text-xs mb-4">
            <div class="flex justify-between text-gray-500">
                <span>Tanggal</span>
                <span class="font-medium text-gray-700">{{ $transaction->created_at->locale('id')->translatedFormat('d M Y, H:i') }}</span>
            </div>
            <div class="flex justify-between text-gray-500">
                <span>Kasir</span>
            ...
            <div class="flex justify-between text-gray-500">
                <span>Tipe</span>
                <span class="font-medium capitalize {{ $transaction->tipe_transaksi === 'servis' ? 'text-blue-600' : 'text-amber-600' }}">
            ...
            <div class="space-y-2">
                ...
                <div class="flex justify-between items-start text-xs">
                    <div class="flex-1">
                        ...
                        <p class="text-gray-400">{{ $detail->qty }} x Rp {{ number_format($detail->harga_jual_saat_transaksi, 0, ',', '.') }}</p>
                    </div>
                    <span class="font-semibold text-gray-700 ml-2">Rp {{ number_format($detail->subtotal, 0, ',', '.') }}</span>
                </div>
                @endforeach
            </div>
        </div>
